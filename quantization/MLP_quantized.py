...
def load_base_model(model_path, num_classes, device):
    model = PostureSensorMLP(input_dim=6, num_classes=num_classes)
...
def main():
    ...
    calib_loader = DataLoader(PostureSensorDataset(os.path.join(DATA_DIR, "val.csv")), batch_size=BATCH_SIZE, shuffle=False, num_workers=0)
    ...
    log_model_metrics(model_dyn, test_loader, save_path, "MLP_INT8_DYNAMIC", device)
